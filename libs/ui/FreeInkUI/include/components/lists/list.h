...
inline ListRowLayout measureListRow(const DrawTarget &target, AssetResolver *assets,
  ...
  if (props.rowPaddingY >= 0) {
    ...
    // props.subtitleRowPadding is a FLOOR, not a replacement: a subtitle row
    // sized on a one-line rowHeight spares nothing, so without it the text
    // comes out flush against both row edges and its neighbours.
    const int16_t spare = static_cast<int16_t>(rowH - labelLh - subLh);
    const int16_t padding = spare > props.subtitleRowPadding ? spare : props.subtitleRowPadding;
